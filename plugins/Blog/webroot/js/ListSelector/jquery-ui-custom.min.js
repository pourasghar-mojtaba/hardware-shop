...
(function (a) {
	...
	a.widget("ui.autocomplete", {
		...
		_normalize: function (b) {
			if (b.length && b[0].label && b[0].value && b[0].image ) {
				return b
			}
			   return a.map(b, function (val) {
					return {	
						label: val.name,
						value: val.name,
						image: val.image,
		...
		_renderItem: function (b, c) {
			//alert(c.label);
			if(c.image==''){
				return a("<li></li>").data("item.autocomplete", c).append("<a><span class='label'><label>" + c.label + "</label></span></a>").appendTo(b)
			}else{
				
				return a("<li></li>").data("item.autocomplete", c).append("<a><img src='" + c.image + "'><span class='label'><label>" + c.label + "</label></span></a>").appendTo(b)
			}
