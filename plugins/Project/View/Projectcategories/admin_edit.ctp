...
<?php
$items = array();
$items['title'] = __d(__PROJECT_LOCALE,'edit_category');
$items['link'] = array('title'=>__d(__PROJECT_LOCALE,'category_list'),'url'  =>__SITE_URL.'admin/'.__PROJECT.'/projectcategories/index');
...
<div class="col-md-6">
	...
	echo  $this->Form->input('parent_id', array(
			...
			'default'=> $this->request->data['Projectcategory']['parent_id'],
	...
	$user_image = '';
		$width      = 200;
		$height     = 200;
		$image      = $this->request->data['Projectcategory']['image'];
		if(fileExistsInPath(__PROJECT_IMAGE_PATH.$image ) && $image != '' ){
			$user_image = $this->Html->image('/'.__PROJECT_IMAGE_URL.$image,array('width' =>$width,'height'=>$height,'id'=>'blog_thumb_image_'.$this->request->data['Projectcategory']['id']));
		}
		else
		{
