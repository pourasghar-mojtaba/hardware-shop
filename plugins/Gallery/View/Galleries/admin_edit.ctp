...
<?php
$items = array();
$items['title'] = __d(__GALLERY_LOCALE,'edit_gallery');
$items['link'] = array('title'=>__d(__GALLERY_LOCALE,'gallery_list'),'url'  =>__SITE_URL.'admin/'.__GALLERY.'/galleries/index');
...
<div class="col-md-6">
	...
	echo  $this->Form->input('title', array(
			...
			'value'=> $this->request->data['Gallerytranslation']['title']
		));
	?>
	<tr>
		<td colspan="5">
			...
			<table id="expense_table" class="expense_table" cellspacing="0" cellpadding="0" width="500">
				<thead>
					<tr>
						<th>
							<?php echo __d(__GALLERY_LOCALE,'image'); ?>
						</th>
						<th>
							<?php echo __d(__GALLERY_LOCALE,'title'); ?>
						</th>
						<th>
							&nbsp;
						</th>
				...
				<tbody>
					...
					if(!empty($galleryimages)){
						foreach($galleryimages as $galleryimage){
							...
							<td>
							<input type='text' class='form-control' name='data[Galleryimage][title][]' id='title_no_01'
							maxlength='255' value='".$galleryimage['Galleryimagetranslation']['title']."' />
							</td>";
							if(fileExistsInPath(__GALLERY_IMAGE_PATH.$galleryimage['Galleryimage']['image'] ) && $galleryimage['Galleryimage']['image'] != '' ){
								echo "<td><a target='_blank' href= '".__SITE_URL.__GALLERY_IMAGE_URL.$galleryimage['Galleryimage']['image']."' >";
								echo $this->Html->image('/'.__GALLERY_IMAGE_URL.$galleryimage['Galleryimage']['image'],array('id'    =>'image_img','height'=>100));
								echo "</a></td>";
							}
							...
							echo "<input type='hidden' value='".$galleryimage['Galleryimage']['id']."' name='data[Galleryimage][id][]'>";
							echo "<input type='hidden' value='".$galleryimage['Galleryimage']['image']."' name='data[Galleryimage][old_image][]'>";
					...
					else
					{
						...
						<tr>
						<td>
						<input type='file' name='data[Galleryimage][image][]' id='image_no_01' maxlength='255'  />
						</td>
						<td>
						<input type='text' name='data[Galleryimage][title][]' id='title_no_01' maxlength='255'  />
						</td>
						<td>&nbsp;</td>
						</tr>
						";
